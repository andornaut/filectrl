...
/// Declares `TomlKeybindings` (one `KeySpec` per binding) and its
/// `to_bindings` conversion from a `field => Action` table per mode.
macro_rules! keybindings {
    (
        normal { $($n_field:ident => $n_action:ident),+ $(,)? }
        prompt { $($p_field:ident => $p_action:ident),+ $(,)? }
    ) => {
        /// Keybindings from the TOML `[keybindings]` section. Every field is required;
        /// the embedded default config supplies them.
        #[derive(Debug, Deserialize)]
        ...
        impl TomlKeybindings {
            /// Converts TOML fields into (normal, prompt) binding lists.
            fn to_bindings(&self) -> Result<(BindingList, BindingList)> {
                // Hardcoded-only actions, listed so their hardcoded keys reach the action map.
        ...
        impl Action {
            /// How an error names the action: its `[keybindings]` key, or what the key
            /// does for a hardcoded-only action.
...
impl KeyBindings {
    /// Parses every key in `toml` without building the maps or checking conflicts.
    ...
    fn lookup(
        ...
        // Uppercase chars may arrive with or without SHIFT depending on the terminal.
        let toggled = modifiers ^ KeyModifiers::SHIFT;
        if let Some(action) = map.get(&KeyCombo::new(code, toggled)) {
            return Some(*action);
        }
        // The legacy encoding sends Alt+Shift+q as Alt+'Q'; the binding holds 'q'
        // with Shift (the kitty protocol's spelling).
    ...
    /// The action's keys (hardcoded, then configured) joined with "/", e.g. "↓/j".
    ...
    /// The keys of all `actions`, each quoted and joined with " or ", for hints.
...
/// Hardcoded normal-mode keys per action, always active and shown in help.
...
/// Hardcoded prompt-mode keys per action, always active and shown in help.
...
/// Builds the key→action map. Hardcoded keys go in first; a config binding of
/// another action's hardcoded key is an error, since it could never fire.
...
/// Builds action→displayed keys, hardcoded first. A list, not joined, since a
/// key may itself be "/".
fn build_display_map(
    ...
    for (action, combos) in normal.iter().chain(prompt.iter()) {
        ...
        // A configured key may repeat a hardcoded one; each is shown once.
...
fn parse_key_spec(spec: &KeySpec) -> Result<Vec<KeyCombo>> {
    match spec {
        ...
        // An empty list would leave the action with no key to show.
...
/// from the front; the rest is the key name, so `+` is a valid key (`"Ctrl++"`).
fn parse_key_combo(s: &str) -> Result<KeyCombo> {
    ...
    'outer: loop {
        for (prefix, modifier) in PREFIXES {
            // `>` keeps the key name non-empty. `get(..)` returns None rather than
            // panicking when `prefix.len()` is not a char boundary.
    ...
    let key_str = rest;
    // Named keys match in any case. Every name is longer than one character.
    let mut code = match key_str.to_ascii_lowercase().as_str() {
        ...
        // Only "F" plus digits is a function key; "Foo+x" is an unknown key.
        s if s.len() > 1 && s.starts_with('f') && s[1..].bytes().all(|b| b.is_ascii_digit()) => {
            // No terminal emits F0 or above F24.
            let num = s[1..]
                .parse::<u8>()
                .ok()
        ...
        _ if key_str.chars().count() == 1 => {
            ...
            // Terminals send Shift+letter as the uppercase character, so "Shift+q" is
            // "Q". A letter whose uppercase is several characters (ß) has no shifted key.
            ...
            // Any other character arrives already shifted ("!" for Shift+1), so the
            ...
            // Shift, so that is the only spelling bound; `lookup` maps the legacy
            // Alt+Shift form to it.
...
/// Refuses an uppercase letter with Ctrl or Alt, naming the spelling to use.
...
fn format_key_combo(combo: &KeyCombo) -> String {
    ...
    // Shift is implied by an uppercase letter, and BackTab renders as
    // "Shift+Tab". With Ctrl or Alt it distinguishes Ctrl+Shift+a from Ctrl+a.
...
mod tests {
    ...
    /// The embedded default config's `[keybindings]` section.
    ...
    /// `[keybindings]` overrides deep-merged onto the defaults.
    ...
    fn a_configured_binding_replaces_the_default_for_that_action() {
        ...
        // An override replaces the default binding, leaving the old key unbound.
        assert_eq!(
    ...
    fn a_hint_marks_an_uppercase_key() {
        // "G" alone does not say that it needs Shift.
        let hint = default_keybindings().hint_for(&[Action::SelectLast]);
        assert!(hint.contains("\"G\" (Uppercase)"), "{hint}");
    }

    /// A terminal may report an uppercase letter without SHIFT.
    #[test]
    ...
    /// The legacy encoding reports Alt+Shift+q as 'Q' with Alt, with or without
    /// Shift.
    ...
    /// Hardcoded-only actions reach the action maps only through the placeholders
    /// `to_bindings` seeds.
    #[test]
    ...
    fn normal_mode_tab_resolves_to_goto() {
        // Tab is hardcoded only in prompt mode; normal mode binds it to goto.
        let kb = default_keybindings();
        assert_eq!(
