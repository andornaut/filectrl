...
- Two pasted entries with the same name (marks from search results can span directories) never collide with each other: like `mv a/x b/x dest/`, the first takes the name and the second is refused, whatever the standing answer, so neither replaces what the other pasted. This holds for names that differ only where the filesystem does not tell them apart (letter case on macOS, for example): no part of a paste replaces an entry another part of it wrote, unless that entry has another hard link, which keeps it.
